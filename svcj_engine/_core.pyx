...
# We reference the header file. 
# setup.py's include_dirs guarantees this is found.
...
def generate_svcj_factor_matrix(object log_returns_df, int window_size, int step_size):
    ...
    # Reshape: (Rolls, Assets, Factors)
    # Note: We reshape directly to match C layout, then Transpose if needed.
    # C layout was: Roll -> Asset -> Factors
    cdef np.ndarray[np.float64_t, ndim=3] drift_tensor = \
        output_buffer[:actual_rolls * total_A * 8].reshape(actual_rolls, total_A, 8)
    
    # The Tensor is currently (Time, Asset, Factor)
    # We need to flatten to (Time, Asset*Factor)
    cdef np.ndarray[np.float64_t, ndim=2] final_matrix = \
        drift_tensor.reshape(actual_rolls, -1)
